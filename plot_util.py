...
#         data = result["ep_score_mean"]
#     else:
#         data = [score for step, score in zip(result["iteration"], result["ep_score_mean"]) if step/1000/1000 < x_lim]
#     return np.percentile(data,95)
...
#         plt.plot(xs, ys, alpha=0.2*alpha, c=color)
#         plt.plot(xs, smooth(ys,smooth_factor), label=run_label if alpha == 1.0 else None, alpha=alpha, c=color, linestyle=ls)
#         plt.xlabel(x_axis)
#         plt.ylabel(y_axis)
...
#
#
# def read_combined_log(path: str, key: str):
#     """
#     Load multiple games and average their scores
#     """
#
...
#     c = 1.7093517175190982
#
#     epoch_scores = defaultdict(
#         lambda: {game: [] for game in game_list}
#     )
#
...
#             print(f"Skipping {game}")
#             continue
#         for env_step, ep_score in zip(game_log["env_step"], game_log["ep_score_mean"]):
#             epoch_scores[env_step // 1e6][game].append(ep_score)
#
...
#         weighted_score = c
#         for game, weight in zip(game_list, game_weights):
#             score = np.mean(es[game])
#             norm_score = asn.normalize(game, score)
#
#             result[f"{game.lower()}_score"].append(score)
#             result[f"{game.lower()}_norm"].append(norm_score)
#             weighted_score += weight * norm_score
